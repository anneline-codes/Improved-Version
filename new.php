...
function load_bins(): array {
    // ── 1. Load seed/meta from bins/ path ──
    $seed_bins = [];
    $raw_bins  = rtdb_request('GET', 'bins');
    if (is_array($raw_bins)) {
        foreach ($raw_bins as $bin_id => $val) {
            if (is_array($val)) {
                $val['id'] = $bin_id;
                $seed_bins[$bin_id] = $val;
            }
        }
    }

    // ── 2. Load AI detections from detections/ path ──
    $raw_detections = rtdb_request('GET', 'detections');
    $detection_bins = [];

    if (is_array($raw_detections)) {
        foreach ($raw_detections as $bin_id => $entries) {
            if (!is_array($entries)) continue;

            // Each entry is a push-key → detection record
            // Sort by key (Firebase push keys are chronological)
            ksort($entries);
            $all_detections = [];
            $latest = null;

            // Aggregate all detections for this bin
            $waste_counts   = [];
            $fill_sum       = 0;
            $fill_count     = 0;
            $confidence_sum = 0;
            ...
            foreach ($entries as $push_key => $entry) {
                ...
                $latest = $parsed; // last one is latest (keys are sorted)
            ...
            // Dominant waste type
            arsort($waste_counts);
            $dominant_waste = array_key_first($waste_counts) ?? 'Mixed';
            $dominant_waste = ucfirst($dominant_waste);

            // Latest fill level (most recent detection)
            $current_fill = isset($latest['fill']) ? (int)round($latest['fill']) : null;

            // Average confidence across all detections
            $avg_confidence = $conf_count > 0 ? round($confidence_sum / $conf_count, 1) : null;

            // Status from fill
            $status = 'ok';
            if ($current_fill !== null) {
                $status = $current_fill >= 80 ? 'critical' : ($current_fill >= 60 ? 'warning' : 'ok');
            }

            // AI prediction from latest, or auto-generate
            $ai_pred = $latest['ai_prediction'] ?? null;
    ...
    // ── 3. Merge: detection data overrides seed data ──
    $merged = [];

    // Start with all seed bins
    foreach ($seed_bins as $bin_id => $seed) {
        $det = $detection_bins[$bin_id] ?? [];
        $merged[$bin_id] = array_merge($seed, $det); // detection wins on conflicts
        $merged[$bin_id]['id'] = $bin_id;
        // Ensure fill has a value
        if (!isset($merged[$bin_id]['fill'])) $merged[$bin_id]['fill'] = 0;
        // Re-compute status from actual fill
        $f = (int)($merged[$bin_id]['fill']);
        $merged[$bin_id]['status'] = $f >= 80 ? 'critical' : ($f >= 60 ? 'warning' : 'ok');
    }

    // ── Known bin locations (used when a bin appears in detections but
    //    has no matching record in bins/).
    //    Add your real bin GPS coordinates here as you deploy hardware.
    // ──────────────────────────────────────────────────────────────────
    $known_locations = [
        'BIN-001' => ['name'=>'Kigali City Tower',  'lat'=>-1.9441,'lng'=>30.0619,'zone'=>'Central','maintenance_score'=>78],
        'BIN-002' => ['name'=>'Kimironko Market',   'lat'=>-1.9355,'lng'=>30.0878,'zone'=>'East',   'maintenance_score'=>92],
        'BIN-003' => ['name'=>'Remera Bus Stop',    'lat'=>-1.9489,'lng'=>30.1079,'zone'=>'East',   'maintenance_score'=>45],
        'BIN-004' => ['name'=>'Nyamirambo Center',  'lat'=>-1.9831,'lng'=>30.0385,'zone'=>'South',  'maintenance_score'=>88],
        'BIN-005' => ['name'=>'Kacyiru Ministry',   'lat'=>-1.9267,'lng'=>30.0653,'zone'=>'North',  'maintenance_score'=>71],
        'BIN-006' => ['name'=>'Gisozi Memorial',    'lat'=>-1.9196,'lng'=>30.0524,'zone'=>'North',  'maintenance_score'=>95],
        'BIN-007' => ['name'=>'UTC Shopping Mall',  'lat'=>-1.9500,'lng'=>30.0588,'zone'=>'Central','maintenance_score'=>63],
        'BIN-008' => ['name'=>'Sonatubes Junction', 'lat'=>-1.9622,'lng'=>30.0731,'zone'=>'Central','maintenance_score'=>81],
        'BIN-009' => ['name'=>'Gikondo Industry',   'lat'=>-1.9750,'lng'=>30.0820,'zone'=>'South',  'maintenance_score'=>52],
        'BIN-010' => ['name'=>'Kinyinya Sector',    'lat'=>-1.9100,'lng'=>30.0950,'zone'=>'North',  'maintenance_score'=>89],
    ];

    // Add any bins that exist ONLY in detections (no seed record)
    foreach ($detection_bins as $bin_id => $det) {
        if (!isset($merged[$bin_id])) {
            // Use known location if available, otherwise default to city centre
            $loc = $known_locations[$bin_id] ?? [
                'name'              => $bin_id,
                'lat'               => -1.9500,
                'lng'               => 30.0619,
                'zone'              => 'Kigali',
            ...
            $merged[$bin_id] = array_merge($loc, $det);
            $merged[$bin_id]['id'] = $bin_id;

            // Auto-register this bin in RTDB bins/ so it persists with location
            // Only write if it truly doesn't exist yet (avoid infinite writes)
            rtdb_set('bins', $bin_id, array_merge($loc, [
                'fill'           => $det['fill'] ?? 0,
                'status'         => $det['status'] ?? 'ok',
                'type'           => $det['type'] ?? 'Mixed',
                'ai_prediction'  => $det['ai_prediction'] ?? 'Normal',
                'last'           => $det['last'] ?? 'Just now',
...
(function(){
  ...
  window._mapMarkers = {};
  const bins = <?=json_encode(array_values(array_map(fn($b)=>['id'=>$b['id']??'','name'=>$b['name']??'','lat'=>(float)($b['lat']??0),'lng'=>(float)($b['lng']??0),'fill'=>(int)($b['fill']??0),'zone'=>$b['zone']??'','type'=>$b['type']??'','status'=>$b['status']??'ok','last'=>$b['last']??'—','ai_prediction'=>$b['ai_prediction']??'—','confidence'=>$b['avg_confidence']??null,'det_count'=>$b['detection_count']??0],$bins)))?>;
  bins.forEach(b => {
    // Skip bins with no coordinates
    if (!b.lat || !b.lng) return;
    const c = b.fill>=80?'#e53e3e':b.fill>=60?'#ff8c00':'#4caf50';
    const icon = L.divIcon({html:`<div style="background:${c};width:32px;height:32px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:3px solid #fff;box-shadow:0 3px 12px rgba(0,0,0,.3);display:flex;align-items:center;justify-content:center;"><span style="transform:rotate(45deg);font-size:10px;font-weight:800;color:#fff;">${b.fill}%</span></div>`,className:'',iconSize:[32,32],iconAnchor:[16,32]});
    ...
    marker.bindPopup(`
      ...
      <div class="popup-row"><span>Last</span><strong>${b.last||'—'}</strong></div>
